...
d3.json('../../data/geoJson/Community_Boundaries.geojson', (jsonData) => {
    ...
    graphPlot.on('mouseover',function(d){
        let dataIsAvailable = !!communities[d.properties.comm_code];
        let tooltipHeight = 20;
        let tooltipWidth = 0;
        let xPosition = d3.mouse(this)[0] + 10;
    ...
    /*
    // draw legend
    var legend2 = graphPlot.selectAll(".legend2")
        .enter().append("g")
        .attr("class", "legend2")
...
function createPlot(communities){
    ...
    // setup x
    var xScale = d3.scaleBand().range([0,(keys.length-1)*MULTIPLIER]),// value -> display
        xAxis = d3.axisBottom().scale(xScale).tickSize(0);
    // setup y
    var yValue = function(d) { return d.bicycle}, // data -> value
